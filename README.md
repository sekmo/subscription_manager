...
**This app is using Rails 7.2.1 and Ruby 3.3.6**. The screenshot doesn't get
...
```sh
git clone https://github.com/nickjj/docker-rails-example subscription_manager
...
Connect to 127.0.0.1:5499, with the user `subscription_manager` and the password `password`.
...
The app is named `subscription_manager` right now but chances are your app will be a different
name. Since the app is already created we'll need to do a find / replace on a
few variants of the string "subscription_manager" and update a few Docker related resources.
